...
public class AI : MonoBehaviour
{
    ...
    public IEnumerator EnemyTurn()
    {
        ...
        while (Array.Exists(handCardList, card => card.cardModel.cost <= gameManager.enemyManaCost))
        {
            // コスト以下のカードリストを取得
            CardController[] selectableHandCardList = Array.FindAll(handCardList, card => card.cardModel.cost <= gameManager.enemyManaCost);
            // 場に出すカードを選択
